...
ISimplePlayer::ISimplePlayer(const PlayerOptions& options, const file_string_path_t& absolute_font_path)
    : StreamHandler(),
      renderer_(nullptr),
      font_(nullptr),
      options_(options),
      ...
      absolute_font_path_(absolute_font_path) {
...
void ISimplePlayer::SetUrlLocation(media::stream_id sid,
...
void ISimplePlayer::HandlePreExecEvent(gui::events::PreExecEvent* event) {
  ...
  if (inf.code == EXIT_SUCCESS) {
    ...
    if (!absolute_font_path_) {
      WARNING_LOG() << "Couldn't open font file path invalid!";
      return;
    }

    const std::string absolute_font_path_str = absolute_font_path_->GetPath();
    const char* font_path_ptr = absolute_font_path_str.c_str();
    font_ = TTF_OpenFont(font_path_ptr, 24);
    if (!font_) {
      WARNING_LOG() << "Couldn't open font file path: " << absolute_font_path_str;
    }
  }
...
media::VideoState* ISimplePlayer::CreateStream(media::stream_id sid,
                                               const common::uri::GURL& uri,
                                               media::AppOptions opt,
                                               media::ComplexOptions copt) {
  if (!uri.is_valid()) {
