...
common::Error UploadTexture(SDL_Texture* tex, const AVFrame* frame) {
  if (frame->format == AV_PIX_FMT_YUV420P) {
    ...
    if (SDL_UpdateYUVTexture(tex, nullptr, frame->data[0], frame->linesize[0], frame->data[1], frame->linesize[1],
    ...
    if (frame->linesize[0] < 0) {
      if (SDL_UpdateTexture(tex, nullptr, frame->data[0] + frame->linesize[0] * (frame->height - 1),
                            -frame->linesize[0]) != 0) {
        return common::make_error(common::MemSPrintf("UpdateTexture error: %s.", SDL_GetError()));
      }
      return common::Error();
    }
    if (SDL_UpdateTexture(tex, nullptr, frame->data[0], frame->linesize[0]) != 0) {
      return common::make_error(common::MemSPrintf("UpdateTexture error: %s.", SDL_GetError()));
    }
