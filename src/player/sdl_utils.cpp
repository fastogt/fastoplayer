...
bool init_audio_params(int64_t wanted_channel_layout, int freq, int channels, media::AudioParams* audio_hw_params) {
  ...
  laudio_hw_params.frame_size =
      av_samples_get_buffer_size(nullptr, laudio_hw_params.channels, 1, laudio_hw_params.fmt, 1);
  laudio_hw_params.bytes_per_sec =
      av_samples_get_buffer_size(nullptr, laudio_hw_params.channels, laudio_hw_params.freq, laudio_hw_params.fmt, 1);
...
bool audio_open(void* opaque,
  ...
  while (!(laudio_dev = SDL_OpenAudioDevice(nullptr, 0, &wanted_spec, &spec,
